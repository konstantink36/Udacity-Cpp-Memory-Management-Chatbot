...
bool ChatBotApp::OnInit()
{
    ...
    ChatBotFrame *chatBotFrame = new ChatBotFrame(wxT("Udacity ChatBot"));  //instantiates the frame
...
// wxWidgets FRAME
ChatBotFrame::ChatBotFrame(const wxString &title) : wxFrame(NULL, wxID_ANY, title, wxDefaultPosition, wxSize(width, height))	//constructor of the frame
{
    // create panel with background image
    ChatBotFrameImagePanel *ctrlPanel = new ChatBotFrameImagePanel(this);

    // create controls and assign them to control panel		
    _panelDialog = new ChatBotPanelDialog(ctrlPanel, wxID_ANY);	 //instantiates chatbot panel dialog
    ...
    // create vertical sizer for panel alignment and add panels. Adds some gui elements
...
void ChatBotFrame::OnEnter(wxCommandEvent &WXUNUSED(event))  // opener capture after we entered text and press enter
{
    ...
    // send user text to chatbot 
     _panelDialog->GetChatLogicHandle()->SendMessageToChatbot(std::string(userText.mb_str()));  //send text to chatbot
}
...
ChatBotPanelDialog::ChatBotPanelDialog(wxWindow *parent, wxWindowID id)		// ChatBotPanelDialog gets instantiated
    : wxScrolledWindow(parent, id)
{
    ...
    _chatLogic->LoadAnswerGraphFromFile(dataPath + "src/answergraph.txt");  //file has all info on nodes and edges
